...
module.exports = selectTimeFindPlace = async function (req, res, next) {
  ...
  const getMonthUserOrderSql = `Select d.user_no,user_name,role,used_time from (select * from (select * from user) as a left join (select * from init_power_role_setting) as b on a.user_power = b.power) as c right join (select * from user_used_order_time where year_and_month = ?) as d on c.user_no = d.user_no order by used_time desc Limit 6`;
  ...
  try {
    ...
    const userListRes = await conn.query(getMonthUserOrderSql, [
