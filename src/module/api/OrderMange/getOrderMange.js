...
module.exports = selectTimeFindPlace = async function (req, res, next) {
  ...
  const selectOrderPage = `SELECT * FROM order_user_apply LIMIT ?,?`;
  const getSumOrder = `SELECT count(order_id) as num FROM order_user_apply `;
  ...
  try {
    ...
    const [placeNumberRes] = await conn.query(getSumOrder);
    res.send({
