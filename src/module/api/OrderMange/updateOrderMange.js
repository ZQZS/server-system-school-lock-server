...
module.exports = selectTimeFindPlace = async function (req, res, next) {
  ...
  const updateOrderSql = `UPDATE order_user_apply SET order_place_id = ?, order_place_password = ?, order_state = ?, order_reject_reason = ?, order_rejecter = ?,order_last_updateing_time =?  WHERE order_id = ?`;
  ...
  try {
    ...
    const result = await conn.query(updateOrderSql, [
      req.body.order_place_id,
      req.body.order_place_password,
